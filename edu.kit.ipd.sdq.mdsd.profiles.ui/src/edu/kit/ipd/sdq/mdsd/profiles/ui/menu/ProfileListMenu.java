...
public class ProfileListMenu extends CompoundContributionItem {
    ...
    public static void createOrUpdateMenuForEachProfile(ITreeSelection treeSelection) {
        ...
        if (firstElement instanceof EStereotypableObject) {
            if (firstTime) {
                ...
                AbstractContributionFactory viewMenuAddition = new AbstractContributionFactory(Constants.MENU_LOCATION,
                        null) {
                    @Override
                    public void createContributionItems(final IServiceLocator serviceLocator,
                            IContributionRoot additions) {
                        Collection<IProfileProvider> registeredProfileProviders = IProfileRegistry.INSTANCE
                        ...
                        for (final IProfileProvider profileProvider : registeredProfileProviders) {
                            ...
                            MenuManager submenu = new MenuManager(profileDescription, Constants.PROFILE_LIST_MENU_ID);
                            IContributionItem dynamicItem = new CompoundContributionItem(Constants.DYNAMIC_LIST_ID) {
                                protected IContributionItem[] getContributionItems() {
                                    IContributionItem[] contributionItems = new IContributionItem[1];
                                    ProfileSubmenu profileSubmenu = new ProfileSubmenu(profileProvider.getProfile());
                            ...
                            // FIXME add a visible when clause so that the menu
